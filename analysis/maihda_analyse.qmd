---
title: "MAIHDA-Analyse: Prozessdarstellung bis zum Nullmodell"
format:
  html:
    toc: true
    toc-depth: 3
...
editor: visual
lang: de
site-url: https://intermind.ch/analyse/
---
...
## Interaktive Karte mit allen Surveys (leaflet + MarkerCluster)

```{r}
#| label: map-interactive
#| code-fold: true
# Quelle: gemergtes Objekt → dat

library(leaflet)

geo_df <- dat |>
  dplyr::select(location_lat, location_lng, survey_created_at,
                device_id, activity, general_wellbeing) |>
  dplyr::filter(!is.na(location_lat), !is.na(location_lng)) |>
  dplyr::mutate(
    wb        = as.numeric(general_wellbeing),
    ts_parsed = suppressWarnings(as.POSIXct(survey_created_at)),
    ts_fmt    = ifelse(is.na(ts_parsed), as.character(survey_created_at),
                       format(ts_parsed, "%Y-%m-%d %H:%M")),
    act       = ifelse(is.na(activity), "", as.character(activity))
  )

cat(sprintf("%s Surveys mit Geo-Daten gefunden.\n", nrow(geo_df)))

if (nrow(geo_df) > 0) {
  center_lat <- mean(geo_df$location_lat)
  center_lng <- mean(geo_df$location_lng)

  leaflet(data = geo_df) |>
    addProviderTiles(providers$CartoDB.Positron) |>
    setView(lng = center_lng, lat = center_lat, zoom = 9) |>
    addCircleMarkers(
      lng = ~location_lng, lat = ~location_lat,
      radius = 4, weight = 1, color = "#3388ff", fill = TRUE, fillOpacity = 0.7,
      popup = ~sprintf("<b>%s</b><br>wb = %.2f<br>%s<br>device = %s",
                       act, wb, ts_fmt, device_id),
      clusterOptions = markerClusterOptions()
    )
} else {
  gt::gt(tibble::tibble(Hinweis = "Keine Geo-Daten vorhanden."))
}
```
...
#| label: dataload-cols
#| code-fold: true
# Ausgabe: Spaltenübersicht (reine Darstellung)
bind_rows(
  tibble(Datensatz = "survey_answers.csv", Spalte = names(survey)),
  tibble(Datensatz = "people_data.csv",    Spalte = names(people))
) |>
  gt() |>
  tab_header(title = "Verfügbare Spalten in den Datensätzen")
```

```{r}
#| label: dataload-qc
#| code-fold: true
# Ausgabe: Merge-Qualität (reine Darstellung)
dat |>
  summarise(
    n_beobachtungen = n(),
    n_personen      = n_distinct(device_id),
    n_strata        = n_distinct(stratum_id),
    na_device_id    = sum(is.na(device_id))
  ) |>
  gt() |>
  tab_header(title = "Datenqualität nach Zusammenführung")
```
