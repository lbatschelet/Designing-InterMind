...
ggsave("plots/survey_counts.png", p_counts, width = 8/2.54, height = 5/2.54, dpi = 300)
```
...
for (col in names(cat_pct_list)) {
  ...
  ggsave(sprintf("plots/cat_dist_%s.pdf", col), p,
         ...
         device = cairo_pdf)
  ggsave(sprintf("plots/cat_dist_%s.png", col), p,
         width  = if (col == "indoors_outdoors") 5/2.54 else 10/2.54,
         height = if (col == "indoors_outdoors") 3/2.54 else 8/2.54,
         dpi = 300)
}
...
make_slider_plot <- function(data_long, var_id, left_lab, right_lab, title_txt, file) {
  ...
  # Speichere auch als PNG
  png_file <- gsub("\\.pdf$", ".png", file)
  ggplot2::ggsave(png_file, p, width = 7.5/2.54, height = 6.5/2.54, dpi = 300)
}
...
strata_table <- strata_overview |>
  ...
  tab_options(
    ...
    table.font.names = "Fira Sans Light"
  ) |>
  opt_table_font(font = "Fira Sans Light")

# Zeige die Tabelle an
print(strata_table)

strata_table_transp <- strata_table |>
  # Tabelle selbst transparent
  tab_options(table.background.color = "transparent") |>
  # auch die HTML-Body-Fläche transparent machen
  opt_css(":root, body { background-color: rgba(0,0,0,0) !important; }")

gtsave(
  strata_table_transp,
  filename = "Plots/strata_overview_table.png",
  vwidth = 1200, vheight = 800, expand = 10
  # Tipp: je nach gt-Version funktioniert auch:
...
# Speichere direkt als PNG
gtsave(table_plot, "Plots/random_slopes_table.png", 
       expand = 10, vwidth = 1200, vheight = 800)
```